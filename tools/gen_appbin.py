...
def gen_appbin():
    ...
    if boot_mode == '2':
        # write irom bin head
        data_bin = struct.pack('<BBBBI', BIN_MAGIC_IROM,
                               4, byte2, byte3, int(entry_addr, 16))
    ...
    data_bin = struct.pack('<BBBBI', BIN_MAGIC_FLASH, 3,
                           byte2, byte3, int(entry_addr, 16))
    sum_size = len(data_bin)
    write_file(flash_bin_name, data_bin)
    ...
    if boot_mode == '1' or boot_mode == '2':
        ...
        write_file(flash_bin_name, chr((all_bin_crc & 0x000000FF)) + chr((all_bin_crc & 0x0000FF00)
                                                                         >> 8) + chr((all_bin_crc & 0x00FF0000) >> 16) + chr((all_bin_crc & 0xFF000000) >> 24))
